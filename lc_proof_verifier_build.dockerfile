...
COPY lc-proof-verifier lc-proof-verifier

# Set compile-time environment variables
ENV CARGO_MANIFEST_PATH="lc-proof-verifier/guest/Cargo.toml"
ENV RUSTFLAGS="-C passes=loweratomic -C link-arg=-Ttext=0x00200800 -C link-arg=--fatal-warnings"
ENV CARGO_TARGET_DIR="lc-proof-verifier/guest/target"
...
ARG BITCOIN_NETWORK
COPY --from=build /src/lc-proof-verifier/guest/target/riscv32im-risc0-zkvm-elf/release ../target/riscv-guest/riscv32im-risc0-zkvm-elf/docker/lc-proof-verifier-guest
COPY --from=build /src/lc-proof-verifier/guest/target/riscv32im-risc0-zkvm-elf/release/lc-proof-verifier-guest elfs/${BITCOIN_NETWORK}-lc-proof-verifier-guest