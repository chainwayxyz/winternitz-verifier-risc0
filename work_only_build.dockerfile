...
COPY work_only work_only
COPY core core

# Set compile-time environment variables
ENV CARGO_MANIFEST_PATH="work_only/guest/Cargo.toml"
ENV RUSTFLAGS="-C passes=loweratomic -C link-arg=-Ttext=0x00200800 -C link-arg=--fatal-warnings"
ENV CARGO_TARGET_DIR="work_only/guest/target"
...
ARG BITCOIN_NETWORK
COPY --from=build /src/work_only/guest/target/riscv32im-risc0-zkvm-elf/release ../target/riscv-guest/riscv32im-risc0-zkvm-elf/docker/work-only-guest
COPY --from=build /src/work_only/guest/target/riscv32im-risc0-zkvm-elf/release/work-only elfs/${BITCOIN_NETWORK}-work-only-guest